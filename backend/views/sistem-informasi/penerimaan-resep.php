...
$no=1;
 ...
 ?>
	...
	<div class="col-sm-5">
				...
				<div class='col-md-3'> 
					<a href="laporan-resep?start=<?= $start ?>&end=<?= $end ?>&cek=<?= $cek ?>" target="_blank" id="print_pdf" class="btn btn-danger" style='margin-top:22px;'><i class="fa fa-print" style="width:20px"></i>Print Pdf</a> 
				</div> 
				<div class="col-sm-3">
...
<?php 
$urlDataSearch = Url::to(['sistem-informasi/get-penerimaan-resep']);
$this->registerJs("
	...
	$('input[name=r2]').on('change',function(){
		...
		if(radio == 'today'){
			...
			$('#print_pdf').attr('href', 'laporan-resep?start='+start+'&end='+end+'&cek='+radio);
			...
			$('#print_pdf').attr('href', 'laporan-resep?start='+start+'&end='+end+'&cek=custom');
			$('#start_date').attr('disabled', false);
			$('#end_date').attr('disabled', false);
		}
	...
	$('#start_date').on('change',function(){
		...
		$('#print_pdf').attr('href', 'laporan-resep?start='+start+'&end='+end+'&cek=custom&search='+$('#textsearch').val());
	});
