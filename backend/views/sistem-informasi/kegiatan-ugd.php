...
$no=1;
 ...
 ?>
	...
	<div class="col-sm-5">
				...
				<div class='col-md-3'> 
					<a href="laporan-kegiatan?start=<?= $start ?>&end=<?= $end ?>&cek=<?= $cek ?>" target="_blank" id="print_pdf" class="btn btn-danger" style='margin-top:22px;'><i class="fa fa-print" style="width:20px"></i>Print Pdf</a> 
				</div> 
				<div class="col-sm-3">
...
<?php 
$urlDataSearch = Url::to(['sistem-informasi/get-kegiatan-ugd']);
$this->registerJs("
	...
	$('input[name=r2]').on('change',function(){
		var radio = $(this).val();
		var date = new Date();
		var day = date.gethate();
		var month = date.getMonth();
		var year = date.getFullYear();
		var lasthay = new Date(year, month + 1, 0);
		var lasthayinyear = new Date(year, 12, 0);
		...
		if(radio == 'today'){
			...
			$('#print_pdf').attr('href', 'laporan-kegiatan?start='+start+'&end='+end+'&cek='+radio);
			$('#start_date').attr('disabled', true);
			$('#end_date').attr('disabled', true);
		} else if(radio == 'this_month') {
			start = 1+'-'+nama_bulan[month]+'-'+year;
			end = lasthay.toLocaleFormat('%d-%b-%Y');
			...
			end = lasthayinyear.toLocaleFormat('%d-%b-%Y');
			...
			$('#print_pdf').attr('href', 'laporan-kegiatan?start='+start+'&end='+end+'&cek=custom');
			$('#start_date').attr('disabled', false);
			$('#end_date').attr('disabled', false);
		}
	...
	$('#start_date').on('change',function(){
		...
		$('#print_pdf').attr('href', 'laporan-kegiatan?start='+start+'&end='+end+'&cek=custom&search='+$('#textsearch').val());
	});
