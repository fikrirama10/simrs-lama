    ...
    <div class="row">
		...
		<div class='box box-body'>
			...
			<div class='row'>
				...
				<?= $form->field($radid, 'idpengirim')->dropDownList(ArrayHelper::map(Dokter::find()->all(), 'id', 'namadokter'))->label(false)?>
				<?php }else{ ?>
				<input type="text" placeholder='<?= $model->dokter->namadokter?>' class="form-control" readonly="" aria-invalid="false">
				<?= $form->field($radid, 'idpengirim')->hiddenInput(['maxlength' => true,'readonly'=>true,'value'=>$model->iddokter])->label(false) ?>
				<?php } ?></div>
			</div>
			<div class='row'>
				<div class ='col-md-2 formright'>No RM</div>
				<div class ='col-md-10'><?= $form->field($radid, 'no_rekmed')->textInput(['maxlength' => true,'readonly'=>true,'value'=>$model->no_rekmed])->label(false) ?></div>
			</div>
			<div class='row'>
				<div class ='col-md-2 formright'>Id Rawat</div>
				<div class ='col-md-10'><?= $form->field($radid, 'idrawat')->textInput(['maxlength' => true,'readonly'=>true,'value'=>$model->idrawat])->label(false) ?></div>
			</div>
			<div class='row'>
			...
			<div class='row'>
				<div class ='col-md-2 formright'>Petugas </div>
				<div class ='col-md-8'> <?= $form->field($radid, 'idpemeriksa')->dropDownList(ArrayHelper::map(User::find()->joinWith(['pegawai as pegawai'])->where(['idpriv'=>20])->all(), 'id', 'pegawai.nama_petugas'))->label(false)?></div>
            ...
            <?php foreach ($raddetail as $index => $raddetail): ?>
                ...
                <div class="item panel panel-default"><!-- widgetBody -->
                    ...
                    <div class="panel-body">
                        ...
                        <div class="row">
                        	<div class='col-md-8'>
							 <?= $form->field($raddetail,  "[{$index}]idjenisrad")->dropDownList(ArrayHelper::map(Dafrad::find()->orderBy(['jenispemeriksaan'=>SORT_ASC])->all(), 'id', 'jenispemeriksaan'),['prompt'=>'- Tindakan Rad -'])->label('idtindakan',['class'=>'label-class'])->label()?>
                        	</div>
                        	<div class='col-md-4'>
                        	     <?= $form->field($raddetail,"[{$index}]nofoto")->textInput(['maxlength' => true]) ?>
                        	</div>
