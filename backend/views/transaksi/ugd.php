			...
			?>
				...
				<div class="col-sm-3">
					<a href="laporanrawat?start=<?= $start ?>&end=<?= $end ?>&cek=<?= $cek ?>" id="cek" class="btn btn-warning mup25" style="display:none;">Apply Filter</a>
					  <!-- radio -->
					<a href="report-ugd?start=<?= $start ?>&end=<?= $end ?>&cek=<?= $cek ?>" target="blank" id="print_pdf" class="btn btn-danger" style='margin-top:22px;'><i class="fa fa-print" style="width:20px"></i>Print Pdf</a>
				</div>
			</div>
		</div>
	<div class='box-body'>
...
<?php
$urlDataSearch = Url::to(['transaksi/get-lihatugd-umum']);
$this->registerJs("
	...
	$('input[name=r2]').on('change',function(){
		...
		if(radio == 'today'){
			...
			$('#print_pdf').attr('href', 'report-ugd?start='+start+'&end='+end+'&cek='+radio);
			...
			$('#print_pdf').attr('href', 'report-ugd?start='+start+'&end='+end+'&cek=custom');
			$('#start_date').attr('disabled', false);
			$('#end_date').attr('disabled', false);
		}
	...
	$('#start_date').on('change',function(){
		...
		$('#print_pdf').attr('href', 'report-ugd?start='+start+'&end='+end+'&cek=custom&search='+$('#textsearch').val());
	});
