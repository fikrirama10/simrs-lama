...
JS;
$masalah = MasalahKeperawatan::find()->where(['idrawat'=>$model->id])->all();
$implementasi = MasalahKeperawatanImplementasi::find()->where(['idrawat'=>$model->id])->groupby(['tanggal'])->all();
?>
	...
	<div class='row'>
		<div class='col-md-12'>
            ...
            <ul class="nav nav-tabs pull-right">
              ...
              <li><a href="#tab_implementasi" data-toggle="tab">Implementasi</a></li>
			   <li><a href="#tab_asuhan" data-toggle="tab">Diagnosis & Asuhan Keperawatan</a></li>
            ...
            <div class="tab-content">
				...
				<div class="tab-pane " id="tab_implementasi">
					<h4>Implementasi</h4>
					 <a href='<?= Url::to(['masalah-keperawatan-implementasi/create?id='.$model->id])?>' class="btn btn-success btn-md" target='_blank'>+ Pemeriksaan</a>
					 <hr>
					 <table class='table table-bordered'>
						<?php foreach($implementasi as $im){ 
						$impl = MasalahKeperawatanImplementasi::find()->where(['idrawat'=>$model->id])->andwhere(['tanggal'=>$im->tanggal])->all();
						?>
							<tr>
								<th class='bg-maroon' colspan='3'><?= $im->tanggal ?></th>
							</tr>
							<tr>
								<th>Jam</th>
								<th>Implementasi</th>
							...
							<?php foreach($impl as $pl){ ?>
							<tr class='bg-navy'>
								<td><?= $pl->jam ?></td>
								<td><?= $pl->asu->implementasi ?> 
								<?php if($pl->implementasi == null){echo'';}else{ ?>
								<b>(<?= $pl->implementasi?>)</b>
								<?php } ?>
								</td>
								<td></td>
				...
				<div class="tab-pane " id="tab_asuhan">
					<h4>Diagnosis & Asuhan Keperawatan</h4>
					 <a href='<?= Url::to(['masalah-keperawatan/create?id='.$model->id])?>' class="btn btn-success btn-md" target='_blank'>+ Pemeriksaan</a>
					 <hr>				
						
					<h4 align='center'>Masalah - Masalah Keperawatan</h4>
					<hr>
					<?php foreach($masalah as $ml): ?>
					 <table class='table table-bordered'>
						<tr>
							
							<th colspan='3' class='bg-navy'><a style='color:#fff;' href='<?= Url::to(['masalah-keperawatan/view?id='.$ml->id])?>'><?= $ml->kategori->kategori ?> - <?= $ml->sub->subkategori ?></a> <i class="fa fa-pencil"></i>	</th>					
						</tr>
						<tr>
							<th width=200>Diagnosis</th>
							<td colspan='2'><?= $ml->diagnosis->kode ?> - <?= $ml->diagnosis->diagnosis ?></td>
						</tr>
						<tr>
							<th>Masalah Keperawatan</th>
							<td colspan='2'><?= $ml->subdiagnosis->tindakan ?></td>
						</tr>
						<?php 
						$intervensi = MasalahKeperawatanDetail::find()->where(['idmasalah'=>$ml->id])->all(); 
						$ic = MasalahKeperawatanDetail::find()->where(['idmasalah'=>$ml->id])->count(); 
						?>
						<tr>
							<th rowspan='<?= $ic+1 ?>'>Intervensi</th>
						</tr>
						
						<?php foreach($intervensi as $inter){ 
						$asuhan = MasalahKeperawatanRencanaasuhan::find()->where(['iddetail'=>$inter->id])->all();
						?>
						<tr>
							<td width='250'><?= $inter->intervensi->intervensi ?></td>
							<td><?php foreach($asuhan as $r){  ?>
								<a style='font-size:16px;'>
								<span class="label <?= $r->rencana->kategori->keterangan ?>"><?= $r->rencana->rencanaasuhan ?></span></a>
								<?php } ?>
							</td>
						</tr>
						<?php } ?>
					 </table>
					 <a style='font-size:13px;'>Keterangan : <?php foreach($kategori as $r){ ?>
							<span class="label <?= $r->keterangan ?>"><?= $r->kategori ?></span>
						<?php } ?>
						</a>
					<hr>
					 <?php endforeach; ?>
