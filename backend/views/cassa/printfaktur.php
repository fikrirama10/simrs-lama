...
<div class='detail-pasien'>
	...
	<table>
			...
			$harga20 = ($td->harga * 20)/100 + $td->harga;
			$total20 = ($td->total * 20)/100 + $td->total;
			$harga_total20 = ($harga_total * 20)/100 + $harga_total;
			?>
			<tr>
				<td><?= $no++?></td>
				<td><?php $tarif= Tarif::findOne($td->idtindakan); if($tarif){echo $tarif->nama;}else{echo'Administrasi pasien';} ?></td>
				<td><?= $td->jumlah ?> Kali</td>
				<td align=right>Rp. <?= Yii::$app->algo->IndoCurr($harga20)?></td>
				<td align=right>Rp. <?= Yii::$app->algo->IndoCurr($total20)?></td>
			...
			<tr>
				...
				<td align=right><h3>Rp. <?= Yii::$app->algo->IndoCurr($harga_total20)?><h3></td>
