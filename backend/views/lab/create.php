    ...
    <div class="row">
		...
		<div class='box box-body'>
            ...
            <?= $form->field($labid, 'kodelab')->textInput(['maxlength' => true,'readonly'=>true])->label(false) ?></div>
				...
				<?= $form->field($labid, 'idpengirim')->dropDownList(ArrayHelper::map(Dokter::find()->all(), 'id', 'namadokter'))->label(false)?>
				<?php }else{ ?>
				<input type="text" placeholder='<?= $model->dokter->namadokter?>' class="form-control" readonly="" aria-invalid="false">
				<?= $form->field($labid, 'idpengirim')->hiddenInput(['maxlength' => true,'readonly'=>true,'value'=>$model->iddokter])->label(false) ?>
				<?php } ?></div>
			</div>
