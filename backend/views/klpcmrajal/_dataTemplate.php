				...
				<table class="table table-striped table-bordered">
					...
					<tbody>
						...
						<tr>
							...
							<td><?= $data->idrawat ?></td>
							<td>
							<?php if($data->idpoli == null){echo"<td></td>";}else{?>
							<?= $data->polii->namapoli ?>
							<?php } ?>
							</td>
...
<?php
$urlDataBarangmd = Url::to(['klpcmrajal/getrajal']);
$urlDataBsSearch = Url::to(['klpcmrajal/getsb']);
		...
		$.ajax({
			...
			success: function (data) {
				if(data !== null){
					...
					$('#klpcm-no_rekmed').val(res.no_rekmed);
					$('#klpcm-idrajal').val(res.id);
					$('#klpcm-dpjp').val(res.iddokter);
					$('#klpcm-idpoli').val(res.idpoli);
