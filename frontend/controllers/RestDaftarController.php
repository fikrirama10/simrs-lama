...
use common\models\PendaftaranOnline;
...
class RestDaftarController extends \yii\rest\Controller
{
...
public function behaviors()
    ...
    public function actionKuota(){
		...
		$tgl = date('Y-m-d');
		$tgldaftar = date('N',strtotime($tgl));
		$model = Poli::find()->where(['between','id',1,5])->all();
		$arrdip=array();
		
		foreach ($model as $q){
			$arrdok=array();
			$dokter = Dokter::find()->where(['idpoli'=>$q->id])->andwhere(['aktif'=>1])->all();
			foreach($dokter as $d){
				$kuota = KuotaPasien::find()->where(['tgl'=>$tgl])->andwhere(['iddokter'=>$d->id])->one();
				$jadwal = Jadwaldokter::find()->where(['idhari'=>$tgldaftar])->andwhere(['iddokter'=>$d->id])->one();
				$jc = Jadwaldokter::find()->where(['idhari'=>$tgldaftar])->andwhere(['iddokter'=>$d->id])->count();
				$kc = KuotaPasien::find()->where(['tgl'=>$tgl])->andwhere(['iddokter'=>$d->id])->count();
				if($kc < 1){
					$kp = 0;
					$ks = 0;
				}else{
			...
			$jadwal2 = Jadwaldokter::find()->where(['idhari'=>$tgldaftar])->andwhere(['idpoli'=>$q->id])->all();
			$kuota2 = KuotaPasien::find()->where(['tgl'=>$tgl])->andwhere(['idpoli'=>$q->id])->all();
			$pasien_daftar = Rawatjalan::find()->where(['DATE_FORMAT(tgldaftar,"%Y-%m-%d")'=>$tgl])->andwhere(['idpoli'=>$q->id])->andwhere(['batal'=>NULL])->count();
			$jumlahsisa = 0;
			$jumlahkuota = 0;
			foreach($jadwal2 as $jd){
				$jumlahkuota += $jd->kuota;
			}
			foreach($kuota2 as $sk){
				$jumlahsisa += $sk->sisa;
			}
			if($jumlahsisa == 0){
				$jumlahsisa = $jumlahkuota;
			}
			array_push($arrdip,[
			'id' => $q->id,			
			'poli' => $q->namapoli,		
	...
	public function actionKuotaTgl($tgl){
		$tgldaftar = date('N',strtotime($tgl));
		$model = Poli::find()->where(['between','id',1,5])->all();
		$arrdip=array();
		...
		foreach ($model as $q){
			...
			'tgl' => $tgl,		
			'jumlahkuota' => $jumlahkuota,		
			'jumlahsisa' => $jumlahkuota - $pasien_daftar,		
			'dokter' => $arrdok,		
			]);
		}
	...
	public function actionPoli(){
		$poli = Poli::find()->where(['between','id',1,5])->all();
		$arrdip=array();
		foreach ($poli as $q){
			$arrdok=array();
			$dokter = Dokter::find()->where(['idpoli'=>$q->id])->andwhere(['aktif'=>1])->all();
			$cd = Dokter::find()->where(['idpoli'=>$q->id])->andwhere(['aktif'=>1])->count();
			foreach ($dokter as $d){
				$arrjadwal=array();
				$jadwal = Jadwaldokter::find()->where(['iddokter'=>$d->id])->orderBy(['idhari'=>SORT_ASC])->all();
				foreach ($jadwal as $j){
					array_push($arrjadwal,[
					'id' => $j->id,			
	...
	public function actionCekJadwal($dokter,$tgl){
		$tanggal = date('Y-m-d',strtotime('+6 hour',strtotime(date('Y-m-d'))));
		$tglawal = strtotime($tgl);
		$tglakhir = strtotime($tanggal);
		$tgldaftar = date('N',strtotime($tgl));
		$jadwal = Jadwaldokter::find()->where(['idhari'=>$tgldaftar])->andwhere(['iddokter'=>$dokter])->count();
		if($tglawal < $tglakhir){
			return (array(
					"metadata"=>array(
						"code"=>401,
						"message"=>'Tanggal Berobat tidak bisa mundur'
					)));
		}else if($tglawal == $tglakhir){
				return (array("metadata"=>array(
						"code"=>401,
						"message"=>'Tanggal Berobat Maksimal H-1'
					)));
		}else{
			if($jadwal < 1){
				return (array(
				"metadata"=>array(
					"code"=>401,
					"message"=>'Tidak ada jadwal di tgl tersebut'
				)));
			}else{
				$ckuota = KuotaPasien::find()->where(['tgl'=>$tgl])->andwhere(['iddokter'=>$dokter])->count();
				
				if($ckuota > 0){
					$kuota = KuotaPasien::find()->where(['tgl'=>$tgl])->andwhere(['iddokter'=>$dokter])->one();
					if($kuota->sisa < 1){
						return (array(
						"metadata"=>array(
							"code"=>401,
							"message"=>'Kuota di tanggal tersebut sudah penuh'
						)));
					}else{
						return (array("metadata"=>array(
							"code"=>200,
	...
	public function actionRujukan($jenis,$norujukan){
		if($jenis == 1){
			$response = $this->get_content('https://new-api.bpjs-kesehatan.go.id:8080/new-vclaim-rest/Rujukan/'.$norujukan.'');
			$data_json = json_decode($response, true);
			return $data_json;
		}else{
			$response = $this->get_content('https://new-api.bpjs-kesehatan.go.id:8080/new-vclaim-rest/RencanaKontrol/noSuratKontrol/'.$norujukan.'');
			$data_json = json_decode($response, true);
			return $data_json;
		}
		
	}
	public function actionDokterRujukanBpjs($kode,$tgl){
		$response = $this->get_content('https://new-api.bpjs-kesehatan.go.id:8080/new-vclaim-rest/referensi/dokter/pelayanan/2/tglPelayanan/'.$tgl.'/Spesialis/'.$kode);
		$data_json = json_decode($response, true);
		$arrdip=array();
		foreach ($data_json['response']['list'] as $q){
			array_push($arrdip,[
			'kode' => $q['kode'],
			'nama' => $q['nama'],
	...
	public function actionDokterKode($kode){
		$q = Dokter::find()->where(['kodedokter'=>$kode])->one();
		return(array(
			"metadata"=>array(
			"code"=>200,
			"message"=>'success'
	...
	public function actionDokterRujukan($kode){
		$dokter = Dokter::find()->joinWith('poli as poli')->where(['poli.kodepoli'=>$kode])->all();
		$arrdip=array();
		foreach ($dokter as $q){
			array_push($arrdip,[
			'id' => $q->id,
	...
	public function actionCariDokter($id){
		$q = Dokter::find()->where(['id'=>$id])->one();
		return(array(
			"metadata"=>array(
			"code"=>200,
			"message"=>'success'
	...
	public function actionCariPoli($id){
		$q = Poli::find()->where(['id'=>$id])->one();
		return(array(
			"metadata"=>array(
			"code"=>200,
			"message"=>'success'
	...
	public function actionKodePoli($id){
		$q = Poli::find()->where(['kodepoli'=>$id])->one();
		return(array(
			"metadata"=>array(
				"code"=>200,
				"message"=>'success'
	...
	public function actionDataPasien($norm,$tgllahir){
		$pasien = Pasien::find()->where(['no_rekmed'=>$norm])->andwhere(['tanggal_lahir'=>$tgllahir])->one();
		$pc = Pasien::find()->where(['no_rekmed'=>$norm])->andwhere(['tanggal_lahir'=>$tgllahir])->count();
		if($pc < 1){
			return (array(
					"metadata"=>array(
					"code"=>400,
	...
	public function actionPasien($norm){
		$pasien = Pasien::find()->where(['no_rekmed'=>$norm])->one();
		$pc = Pasien::find()->where(['no_rekmed'=>$norm])->count();
		if($pc < 1){
			return (array(
					"metadata"=>array(
					"code"=>400,
	...
	public function actionCekKunjungan(){
		$response=$this->get_content('https://new-api.bpjs-kesehatan.go.id:8080/new-vclaim-rest/monitoring/HistoriPelayanan/NoKartu/0001391429979/tglAwal/2021-02-01/tglAkhir/2021-04-30');
		$data_json=json_decode($response, true);
		$dd = $data_json['response']['histori'];
		$arrdip=array();
		foreach ($dd as $q){
			array_push($arrdip,[
			'Rujukan' => $q['noRujukan'],
...
public function actionStok($bulan='',$bayar='',$tahun=''){
	...
	public function actionKasirYanmasDay(){
		...
		$jmltrxyanmas = Transaksi::find()->where(['MONTH(tglbayar)'=>$day])->andwhere(['idbayar'=>4])->count();	
		$jmltrxbpjs = Transaksi::find()->where(['MONTH(tglbayar)'=>$day])->andwhere(['idbayar'=>5])->count();	
		...
		$rajalumumi = Transaksi::find()->where(['MONTH(tglbayar)'=>$day])->andwhere(['idjenisrawat'=>1])->andwhere(['idbayar'=>4])->all();	
		$igdumumi = Transaksi::find()->where(['MONTH(tglbayar)'=> $day])->andwhere(['idjenisrawat'=>3])->andwhere(['idbayar'=>4])->all();	
		$ranapumumi = Transaksi::find()->where(['MONTH(tglbayar)'=>$day])->andwhere(['idjenisrawat'=>2])->andwhere(['idbayar'=>4])->all();	
	    
	    $rajalbpjsi = Transaksi::find()->where(['MONTH(tglbayar)'=>$day])->andwhere(['idjenisrawat'=>1])->andwhere(['idbayar'=>5])->all();	
		$igdbpjsi = Transaksi::find()->where(['MONTH(tglbayar)'=> $day])->andwhere(['idjenisrawat'=>3])->andwhere(['idbayar'=>5])->all();	
		$ranapbpjsi = Transaksi::find()->where(['MONTH(tglbayar)'=>$day])->andwhere(['idjenisrawat'=>2])->andwhere(['idbayar'=>5])->all();	
...
//fungsi check tanggal merah
	...
	public function actionDaftar(){
		...
		exit("Data empty.");
			...
			$tglberobat = $arr->TglBerobat;
			$iddokter = $arr->IdDokter;
			$idpoli = $arr->IdPoli;
			$email = $arr->Email;
			$notlp = $arr->NoTlp;
			$anggota = $arr->Anggota;
			...
			$model = Pasien::find()->where(['no_rekmed'=>$norm])->one();
			...
			$rawatjalan->genKode();
			$daftar = date('Y-m-d',strtotime($tglberobat));
			($rawatjalan->antrian)? $rawatjalan->antrian : $rawatjalan->genAntri($idpoli,$iddokter,$anggota,$daftar);
			$rawatjalan->idpekerjaan = $model->idpekerjaan;
			$rawatjalan->tgldaftar = $tglberobat;
			$rawatjalan->idpoli = $idpoli;
			$rawatjalan->iddokter = $iddokter;
			$rawatjalan->anggota = $anggota;
			$rawatjalan->no_rekmed = $norm;
			$rawatjalan->norujukan = $norujukan;
			$rawatjalan->idjenisrawat = 1 ;
			$rawatjalan->idbayar = $bayar ;
			$rawatjalan->klpcm = 1 ;
			$rawatjalan->status = 1 ;
			$rawatjalan->idkb = $idkb ;
			...
			$tgldaftar = date('N',strtotime($tglberobat));
			$kuota = KuotaPasien::find()->where(['idhari'])->where(['idhari'=>$tgldaftar])->andwhere(['iddokter'=>$rawatjalan->iddokter])->andwhere(['tgl'=>$tglberobat])->count();
			if($kuota > 0){
				$kp = KuotaPasien::find()->where(['idhari'=>$tgldaftar])->andwhere(['iddokter'=>$iddokter])->andwhere(['tgl'=>$tglberobat])->one();
				$kp->sisa = $kp->sisa - 1;
				$kp->save();
			}else{
				$jadwal2 = Jadwaldokter::find()->where(['idhari'=>$tgldaftar])->andwhere(['iddokter'=>$iddokter])->one();
				$kuotadaftar->tgl = $tglberobat;
			...
			if($rawatjalan->save(false)){				
				...
				return(array(
					"response"=>array(
						"nomorantrean" => $rawatjalan->antrian,
					),
					"metadata"=>array(				
						"massage"=>"Ok",
						"code"=>200,
