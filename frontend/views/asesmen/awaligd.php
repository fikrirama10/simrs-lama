...
$pekerjaan = Pekerjaan::find()->where(['idpasien'=>$model->no_rekmed])->one();
...
<?= $form->field($model, 'iddokter')->dropDownList(ArrayHelper::map(Dokter::find()->where(['idpoli'=>null])->all(), 'id', 'namadokter'),['prompt'=>'- Pilih Dokter -'])->label('Dokter',['class'=>'label-class'])->label("Dokter Pemeriksa")?>
	...
	<div class='col-md-4'>
		<div class="box box-primary">
			...
			<hr>
              ...
              <hr>
			  	 <strong><i class=""></i> Pekerjaan</strong>
			<?php if($pekerjaan == null ){echo" <h3>Belum PPI</h3> <div class='timeline-footer'></div>";}else{ ?>
              <p class="text-muted"><?= $pekerjaan->kerja->jenis?>
			  <?php if($pekerjaan->idjenis < 2){echo"(<a>NRP : ".$pekerjaan->nrp."<a>) , <a>(Pangkat : ".$pekerjaan->pangkat.")</a>, <a>(Kesatuan : ".$pekerjaan->kesatuan.")</a>";}else{echo"";}?>
			  </p>
			<?php } ?>
			  

              <hr>
			   <strong><i class=""></i>Jenis Bayar</strong>

              <p class="text-muted"><?= $model->carabayar->jenisbayar?></p>

              <hr>
			   <strong><i class=""></i> Penanggung jawab</strong>

              <p class="text-muted"><?= $model->penanggung?> (Hubungan <?= $model->hub->hubungan?>)</p>
	...
	<div class='col-md-8'>
		<div class="nav-tabs-custom">
            <ul class="nav nav-tabs pull-right">
              <li><a href="#tab_1-4" data-toggle="tab">Tindak Lanjut</a></li>
              <li><a href="#tab_1-3" data-toggle="tab">Obat</a></li>
              <li><a href="#tab_1-2" data-toggle="tab">Tindakan</a></li>
              <li><a href="#tab_1-1" data-toggle="tab">Diagnosis Awal</a></li>
              <li><a href="#tab_2-2" data-toggle="tab">Pemeriksaan Fisik</a></li>
              <li><a href="#tab_3-2" data-toggle="tab">Anamnesis</a></li>
              <li class="active"><a href="#tab_4-2" data-toggle="tab">Triage</a></li>
            ...
            <div class="tab-content">
               ...
               <h3> Anamnesa / Anamnesis</h3>
					...
					<div class='row'>
						<div class='col-md-12'>
							...
							<?= $form->field($keluhan, 'rwt_penyakitd')->textarea(['rows' => '4','required'=>true])->label("Riwayat Penyakit Dulu")?>
              ...
              <div class="tab-pane " id="tab_2-2">
                 <h3>Pemeriksaan Fisik</h3>
				  <?= $form->field($rxfisik, 'keadaan')->dropDownList(ArrayHelper::map(Keadaan::find()->all(), 'id','keaddan'))?>
				  ...
				  <?= $form->field($rxfisik, 'rx_fisik')->textarea(['rows'=>'6','required'=>true])->label("Status Lokalis") ?>
				  ...
				  <?= $form->field($rxfisik, 'suhu')->textinput(['placeholder'=>'Cº']) ?>
				  <?= $form->field($rxfisik, 'nadi')->textinput(['placeholder'=>'']) ?>
              </div>
              <!-- /.tab-pane -->
              <div class="tab-pane" id="tab_1-1">
			  <?= $form->field($model, 'klab')->textinput(['placeholder' => 'KODE DIAGNOSA','onkeyup'=>'$.get("'.Url::toRoute('tes/listdiagnosa/').'",{ id: $(this).val() }).done(function( data ) 
									{
			  ...
			  <div class="tab-pane" id="tab_1-2">
               <h3>Tindakan<a href='<?= Url::to(['asrawatjalanesmen/tddokter/'.$model->id]) ?>' class='btn btn-primary btn-xs pull-right' style='margin-bottom:10px;'>+ Tindakan</a></h3>
			  ...
			  <div class="tab-pane" id="tab_1-4">
               ...
               <h3>Triage</h3>
				...
				<?= $form->field($model, 'triage')->radioList(ArrayHelper::map(Triage::find()->all(), 'id','kategori')); ?>
				</div>
